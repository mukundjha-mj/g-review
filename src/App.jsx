import { useMemo, useState } from 'react'
import './App.css'

const message = 'Excellent service! Very professional and timely. The team was courteous and did a fantastic job. Highly recommend this business for anyone looking for quality service. 👍'

function ReviewCard({ reviewUrl }) {
  const [copied, setCopied] = useState(false)
  const copy = async () => {
    try { await navigator.clipboard.writeText(message); setCopied(true) }
    catch { window.prompt('Copy this review message:', message) }
  }
  const openReview = async () => {
    try { await navigator.clipboard.writeText(message) } catch { /* Copy button remains the reliable fallback. */ }
    window.location.assign(reviewUrl)
  }
  return <main className="page"><section className="card review"><div className="google">G</div><h1>Share your review</h1><p className="subtitle">Thank you! Tell others about your experience.</p><div className="stars">★★★★★</div><p className="label">YOUR REVIEW MESSAGE</p><p className="message">{message}</p><button className="copy" onClick={copy}>{copied ? '✓ Copied! Now tap below' : '📋 Tap to Copy Message'}</button><p className="then">then</p><button className="primary" onClick={openReview}>⭐ Open Google Review &amp; Paste</button><p className="hint">On Google, choose your rating, tap the text box, paste, then post.</p></section></main>
}

function Generator() {
  const [businessUrl, setBusinessUrl] = useState('')
  const [error, setError] = useState('')
  const [qrUrl, setQrUrl] = useState('')
  const [loading, setLoading] = useState(false)

  const generate = async () => {
    setError('')
    let inputUrl
    try { inputUrl = new URL(businessUrl.trim()) } catch { return setError('Paste a valid Google Maps business link.') }
    if (inputUrl.protocol !== 'https:') return setError('Use an https:// Google link.')

    setLoading(true)
    try {
      const response = await fetch(`/api/resolve-review?url=${encodeURIComponent(inputUrl.href)}`)
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Could not prepare the review link.')

      const landing = new URL(location.href)
      landing.search = ''
      landing.searchParams.set('review', data.reviewUrl)
      setQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=500x500&margin=12&data=${encodeURIComponent(landing.href)}`)
    } catch (failure) {
      setQrUrl('')
      setError(failure.message)
    } finally {
      setLoading(false)
    }
  }

  return <main className="page"><section className="card"><h1>Google Review QR Generator</h1><p className="subtitle">Paste your Google Maps business link to create a QR code.</p><label>Google Maps business link<input value={businessUrl} onChange={(event) => setBusinessUrl(event.target.value)} type="url" placeholder="https://maps.app.goo.gl/..." /></label><button className="primary" onClick={generate} disabled={loading}>{loading ? 'Preparing review link…' : 'Generate QR Code'}</button><p className="error" role="alert">{error}</p>{qrUrl && <section className="result"><h2>Scan to leave a review</h2><img src={qrUrl} alt="Review QR code"/><button className="copy" onClick={() => window.print()}>Print QR Code</button></section>}</section></main>
}

export default function App() {
  const query = useMemo(() => new URLSearchParams(location.search), [])
  return query.has('review') ? <ReviewCard reviewUrl={query.get('review')} /> : <Generator />
}